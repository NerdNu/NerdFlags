...
public class NerdFlagsListener implements Listener {
	...
	private final Pattern compassPattern = Pattern.compile("(?i)^/(worldedit:)?/?(" + StringUtils.join(
			Arrays.asList("unstuck", "!", "ascend", "asc", "descend", "desc", "ceil", "thru", "jumpto", "j", "up"), "|")
			+ ")");
	...
	public void onEntityPortal(EntityPortalEvent event) {
		...
		if (fromDimension == Environment.THE_END && toDimension == Environment.NORMAL
				|| toDimension == Environment.THE_END) {
	...
	/**
	 * Listens for player deaths in order to properly handle the keep-inventory
	 * region flag.
	 */
